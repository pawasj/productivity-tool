...
import { MessageSquare, Plus, X, User, Clock } from "lucide-react";
...
export default function DiscussionsModule({ verticalId, userId, members }: Props) {
  ...
  async function fetchDiscussions() {
    const { data } = await supabase
      .from("discussions")
      .select("*, profiles!discussions_created_by_fkey(full_name), with_member:profiles!discussions_with_member_id_fkey(full_name)")
  ...
  async function addDiscussion() {
    ...
    setSaving(false);
    setShowForm(false);
    setTitle(""); setDescription(""); setWithMember("");
  }
    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
      <div className="flex items-center justify-between p-4 border-b border-slate-100">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-slate-900 text-sm">Pending Discussions</h3>
          {pending.length > 0 && (
      ...
      {showForm && (
        <div className="p-3 border-b border-slate-100 bg-slate-50 animate-fade-in">
          ...
          <select
            ...
            className="w-full px-3 py-1.5 border border-slate-200 rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-amber-400 mb-2"
          >
            <option value="">With whom? (optional)</option>
            {members.map((m) => (
      ...
      <div className="flex-1 min-w-0">
        ...
        {d.with_member && (
          <div className="flex items-center gap-1 mt-0.5">
            ...
            <span className="text-xs text-slate-400">with {(d.with_member as Profile).full_name}</span>
          </div>
        )}
